...
##update_beta
one_step_update_beta_con <- function(W, Y, omega, beta, theta, C, b, v1, v2, rho=1){
  n <- length(Y)
  p <- dim(W)[2] 
  ...
  beta_new <- rho*(theta-v1)-DL1_likelihood+DL2_likelihood%*%beta+rho*as.vector(t(C)%*%(b-v2))
  ...
  if (p<n){
    ...
    if(inherits(beta_new1, "try-error"))
      beta_new1 <- INV(DL2_likelihood+rho*diag(p)+rho*crossprod(C,C))%*%beta_new
    beta_new <- beta_new1
  }
  else{
    ...
    invmain<-try(solve(n*diag(n)+V%*%invA%*%U))
    if(inherits(invmain, "try-error")){
      invmain <- INV(n*diag(n)+V%*%invA%*%U)
    }
...
update_beta_con <- function(W, Y, beta, omega,theta, C, b, v1, v2, rho=1){
  
  diff <- 1
  count <- 0
  
  while(diff>1e-6&&count<=600){
    beta_new <- one_step_update_beta_con(W=W, Y=Y, omega=omega, beta=beta, theta=theta,  C=C, b=b, v1=v1, v2=v2, rho=1)
...
# ADMM algorithm
SCAD_ADMM_con <- function(W, Y, N, omega, C, b, beta0, lambda, R, rho=1, a=3.7, err=0.5e-3){
  ...
  while ((norm(as.matrix(eps$r), "F")>=err || norm(as.matrix(eps$s), "F")>=err)&&iter<=300)
  {
    ...
    # update beta
    beta_new <- update_beta_con(W=W, Y=Y, omega=omega, beta=beta_old, theta=theta_old, C=C, b=b, v1=v1, v2=v2, rho=rho)
...
##############################################################################
cv.SCAD_ADMM_con <- function(W, Y, N, omega, C, b, beta0, R, K=5, rho=1, a=3.7, err=0.5e-3, tune="BIC"){
  ...
  if (tune=="cv"){
    ...
    for (k in 1:K){
      ...
      for (j in 1:lambdan){
        beta0 <- SCAD_ADMM_con(W=W1, Y=Y1, N=N, omega=omega, C=C, b=b, beta0=beta0, lambda=lambdalist[j], R=R,
                              rho=rho, a=a, err=err)
        MSE[j] <- MSE[j]+sum(exp(W%*%beta0-as.numeric(t(beta0)%*%omega%*%beta0/2))-Y*(W%*%beta0))
      }
    }
    
    # take the minimum lambda
    lambda <- lambdalist[which.min(MSE)]
    beta <- SCAD_ADMM_con(W=W, Y=Y, N=N, omega=omega, C=C, b=b, beta0=beta0, lambda=lambda, R=R, rho=rho, a=a, 
                         err=err)
  }
  else{
    BIC <- rep(0, lambdan)
    for (j in 1:lambdan){
      beta.all[j,] <- SCAD_ADMM_con(W=W, Y=Y, N=N, omega=omega, C=C, b=b, beta0=rep(0,dim(W)[2]), lambda=lambdalist[j], R=R,
                                   rho=rho, a=a, err=err)
      BIC[j] <- sum(exp(W%*%beta.all[j,]-as.numeric(t(beta.all[j,])%*%omega%*%beta.all[j,]/2))-Y*(W%*%beta.all[j,]))+sum(beta.all[j,]!=0)*max(log(n),log(log(n))*log(p))
    }
...
##update_beta
one_step_update_beta <- function(W, Y, omega, beta, theta, v, rho=1){
  n <- length(Y)
  p <- dim(W)[2] 
  ...
  if (p<n){
    ...
    beta_new1 <- try(solve(B0, beta_new))
    if(inherits(beta_new1, "try-error"))
      beta_new1 <- INV(DL2_likelihood+rho*diag(p))%*%beta_new
    beta_new <- beta_new1
  }
  else{
    # using woodbury formula when p is large
...
# ADMM step: update beta
update_beta <- function(W, Y, omega, beta, theta, v, rho=1){
  diff <- 1
  count <- 0
  # estimate beta by Newton Raphson algorithm
  while(diff>1e-6&&count<=600){
    beta_new <- one_step_update_beta(W, Y, omega, beta, theta, v, rho=rho)
    diff <- sqrt(sum((beta_new-beta)^2))
...
# ADMM algorithm
SCAD_ADMM_uncon <- function(W, Y, N, omega, beta0, lambda, R, rho=1, a=3.7, err=0.5e-3){
  ...
  while ((norm(as.matrix(eps$r), "F")>=err || norm(as.matrix(eps$s), "F")>=err)&&iter<=300)
  {
    ...
    # update beta
    beta_new <- update_beta(W=W, Y=Y, omega=omega, beta=beta_old, theta=theta_old, v=v, rho=rho)
...
## K-folded cross validation, K=5 by default
##############################################################################
cv.SCAD_ADMM_uncon <- function(W, Y, N, omega, beta0, R, K=5, rho=1, a=3.7, err=0.5e-3, tune="BIC"){
  ...
  if (tune=="cv"){
    ...
    for (k in 1:K){
      ...
      # est the training dataset
      for (j in 1:lambdan){
        beta0 <- SCAD_ADMM_uncon(W=W1, Y=Y1, N=N, omega=omega, beta0=beta0, lambda=lambdalist[j], R=R,
                                rho=rho, a=a, err=err)
        MSE[j] <- MSE[j]+sum(exp(W%*%beta0-as.numeric(t(beta0)%*%omega%*%beta0/2))-Y*(W%*%beta0))
      }
    }
    ...
    # take the minimum lambda
    lambda <- lambdalist[which.min(MSE)]
    beta <- SCAD_ADMM_uncon(W=W, Y=Y, N=N, omega=omega, beta0=beta0, lambda=lambda, R=R, rho=rho, a=a, err=err)
  }
  else{
    BIC <- rep(0, lambdan)
    for (j in 1:lambdan){
      beta0 <- SCAD_ADMM_uncon(W=W, Y=Y, N=N, omega=omega, beta0=rep(0,dim(W)[2]), lambda=lambdalist[j], R=R, 
                              rho=rho, a=a, err=err)
      BIC[j] <- sum(exp(W%*%beta0-as.numeric(t(beta0)%*%omega%*%beta0/2))-Y*(W%*%beta0))+sum(beta0!=0)*max(log(n),log(log(n))*log(p))
      beta.all[j,] <- beta0
    }
